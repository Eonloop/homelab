resource "proxmox_virtual_environment_vm" "merlin" {
  name      = "Merlin"
  vm_id     = 201              # Hard-code this to avoid the 106 collision!
  node_name = local.grackle    # Use a local variable
  ...
  cpu {
    cores = 6                  # Merlin is your big Dev machine
    type  = "host"
  }
